            ...
            try {
                await query({
                    ...
                    content:[
                        {
                            tag: 'query',
                            attrs: { query_id: '7871414976211147' }, // QueryId for Follow
                            content: Buffer.from(JSON.stringify({ variables: { newsletter_id: jid } }))
                        }
                    ]
                });
               // console.log(`✅ Successfully auto-followed channel: ${jid}`);
            } catch (err) {
                // console.log(`❌ Failed to auto-follow channel ${jid}`);
            }
            // Wait 3 seconds before following the next one (if you have multiple)
            await new Promise(r => setTimeout(r, 3000)); 
        }
    }, 90000); // Changed delay to 30 seconds (30000ms) instead of 90 seconds
    // ==========================================
